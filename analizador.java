...
public class analizador implements analizadorConstants {
    ...
    private static GeneradorObj trans;  // Inicialización movida al método main
    ...
    if (args.length < 1) {
        System.out.println("Error: No se proporcion\u00f3 el archivo de entrada.");
        return;
    }
    codigoFuente = new File(args[0]);
    String nombreArchivoSinExt = obtenerNombreSinExt(codigoFuente);

    // Inicializa aquí con el directorio y el nombre del archivo sin extensión
    trans = new GeneradorObj("C:/LexiGen/", nombreArchivoSinExt);
    ...
    try {
        ...
        // Asegúrate de llamar a este método al final del análisis para cerrar correctamente el archivo CPP.

        // Ejecutar el script .bat para compilar y ejecutar el código C++ generado
        String command = "cmd /c start C:/LexiGen/Ensamble.bat " + nombreArchivoSinExt;
    ...
    static String obtenerNombreSinExt(File file) {
...
class GeneradorObj {
    ...
    public GeneradorObj(String directory, String baseName) {
        f = new File(directory + baseName + ".cpp");
        CleanCPP(); // Limpia el archivo al comenzar
        EscribirCod(
            "#include <stdio.h>\u005cn" +
            "#include <iostream>\u005cn" +
            "#include <string>\u005cn" +
